...
function Load-Certificate {
    ...
    try {
        Write-EnhancedLog -Message "Attempting to load certificate from path: $CertPath" -Level "INFO" -ForegroundColor ([ConsoleColor]::Cyan)
        ...
        Write-EnhancedLog -Message "Successfully loaded certificate from path: $CertPath" -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)

        # Validate certificate path after loading
        $certExistsAfter = Validate-Certificate -CertPath $CertPath
        if ($certExistsAfter) {
            Write-EnhancedLog -Message "Certificate path still exists after loading: $CertPath" -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)
        } else {
            Write-EnhancedLog -Message "Certificate path does not exist after loading: $CertPath" -Level "WARNING" -ForegroundColor ([ConsoleColor]::Yellow)
        }

        return $cert
    }
    catch {
        Write-EnhancedLog -Message "Error loading certificate from path: $CertPath. Error: $_" -Level "ERROR" -ForegroundColor ([ConsoleColor]::Red)
        throw $_
    }
}
